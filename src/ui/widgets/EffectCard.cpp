...
EffectCard_t* effect_card_create(lv_obj_t* parent, int32_t x, int32_t y, 
    ...
    lv_obj_set_style_bg_color(card->card, UiTheme_get_color(COLOR_BG_SECONDARY), 0);
    lv_obj_set_style_border_color(card->card, UiTheme_get_color(COLOR_BORDER), 0);
    lv_obj_set_style_border_width(card->card, 1, 0);
    lv_obj_set_style_pad_all(card->card, 4, 0);
    lv_obj_set_style_radius(card->card, 6, 0);
    ...
    lv_obj_set_style_text_color(card->name_label, UiTheme_get_color(COLOR_TEXT_PRIMARY), 0);
    
    // Indicador de status (LED circular)
    card->status_indicator = lv_obj_create(card->card);
    lv_obj_set_size(card->status_indicator, 8, 8);
    lv_obj_align(card->status_indicator, LV_ALIGN_TOP_RIGHT, 0, 0);
    lv_obj_set_style_bg_color(card->status_indicator, UiTheme_get_color(COLOR_DISABLED), 0);
    ...
    lv_obj_set_style_text_color(card->param_label, UiTheme_get_color(COLOR_ACCENT_BRIGHT), 0);
