...
EffectCard_t* effect_card_create(lv_obj_t* parent, int32_t x, int32_t y, 
    ...
    lv_obj_set_style_bg_color(card->status_indicator, COLOR_EFFECT_LED_OFF, 0);
...
void effect_card_set_enabled(EffectCard_t* card, bool enabled) {
    ...
    // Atualiza cor do LED indicador - cyan bright quando ON, não verde
    lv_obj_set_style_bg_color(card->status_indicator, 
        enabled ? COLOR_EFFECT_LED_ON : COLOR_EFFECT_LED_OFF, 0);
    
    // Atualiza border do card para destacar quando ativo - cyan sutil
    lv_obj_set_style_border_color(card->card, 
        enabled ? COLOR_BORDER_ACTIVE : COLOR_BORDER_SUBTLE, 0);
}
